...
const Chat = () => {
  ...
  const [conversationsMessages, setConversationsMessages] = useState({ 1: [] })
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('policy-updated', handlePolicyUpdate)
  }, [])
  ...
  const handleSendMessage = async (content, attachedFiles = []) => {
    ...
    try {
      ...
      setConversationsMessages((prev) => {
        ...
        return {
          ...prev,
          [chatIdAtSend]: existingMsgs.map((m) =>
            m.id === assistantMsgId
              ? { ...m, content: reply, isStreaming: false, routing: backendResponse }
              : m
          ),
        }
      })
      ...
      let savedAmount = 0
      if (totalTokens > 0) {
        const baselineCost = totalTokens * 0.000003
  ...
  const handleRegenerateResponse = (messageId) => {
    ...
    setConversationsMessages((prev) => ({
      ...prev,
      [activeChatId]: (prev[activeChatId] || []).slice(0, index),
    }))
    const originalFiles = promptMsg.files
      ? promptMsg.files.map((f) => ({ name: f.name, size: f.size, type: f.type }))
      : []
    handleSendMessage(promptMsg.content, originalFiles)
  }

  const handleClearConversation = () => {
    setConversationsMessages((prev) => ({ ...prev, [activeChatId]: [] }))
