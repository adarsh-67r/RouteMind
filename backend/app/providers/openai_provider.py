...
class OpenAIProvider(BaseProvider):
    ...
    def generate_response(self, prompt: str, model: str = None, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            latency_ms = (time.perf_counter() - start_time) * 1000
            logger.error("OpenAI Authentication Error: %s (Latency: %.2f ms)", str(e), latency_ms)
            raise ProviderAuthenticationError(f"OpenAI Authentication failed: {e}") from e
            
        except APIConnectionError as e:
            latency_ms = (time.perf_counter() - start_time) * 1000
            logger.error("OpenAI Connection Error: %s (Latency: %.2f ms)", str(e), latency_ms)
            raise ProviderConnectionError(f"Failed to connect to OpenAI API: {e}") from e
            
        except RateLimitError as e:
            latency_ms = (time.perf_counter() - start_time) * 1000
            logger.error("OpenAI Rate Limit Error: %s (Latency: %.2f ms)", str(e), latency_ms)
            ...
            latency_ms = (time.perf_counter() - start_time) * 1000
            logger.error("Unexpected error in OpenAI provider: %s (Latency: %.2f ms)", str(e), latency_ms)
            raise ProviderAPIError(f"An unexpected error occurred during the OpenAI call: {e}") from e
